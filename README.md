...
Additionally, there are input files within the tests directory are used for the examples in Question 1. To measure their runtimes and place in runtime.csv in runtimes directory use the following command...
...
The above graph shows the runtime of 10 input files from tests/question 1 with increasing sizes from 25 to 230. Initial system noise and measurement variability shows a spike within the first few ms, but overall there's an increase in runtime as input string size increases. Specifically, one that matches O(m*n).
...
To compute the length of HVLCS, the pseudocode would utilize the recurrence relation from question 2. Regarding data structures, it will need a 2D array of length nxm that where n and m represent the lengths of strings A and B repesctively. The array stores the optimal values As the problem asks for the length, backtracking is necessary to find the subsequence's length. 
...
def HVLCS(A, B, v): 
    ...
    for i = 1 to n: 
        for j = 1 to m: 
            #match
            if A[i-1] = B[j-1]:
                OPT[i][j] = max(OPT[i-1][j], OPT[i][j-1], OPT[i-1][j-1] +v(A[i-1]))
            else: 
            #mismatch 
                OPT[i][j] = max(OPT[i-1][j], OPT[i][j-1])

    #backtrack to find sequence and return length(Sequence)
    
    while i > 0 and j > 0:
        #match and max val
        if A[i-1] == B[j-1] and OPT[i][j] == OPT[i-1][j-1] + v(A[i-1]):
            subsequence.append(A-1)
            i = i - 1
            j = j - 1
        #mismatch so move 
